...
impl<T: Interpolatable> Interval<T> {
    ...
    pub fn get(&self, t: f32) -> T {
        ...
        let t = (self.uniform_transformation)(t);
...
impl<T: 'static + Interpolatable> Spline<T> {
    ...
    /// Tension should be in range `0.0..=1.0`.
    /// For example, if tension is `0.5`, then the curve is a Catmull-Rom spline.
    ///
    /// # Panic
    /// Panics if the input `points` is empty.
    ...
    /// with the given interpolation time on that interval.
    ///
    /// Returns `None` if the interval is invalid.
    ///
    /// Interpolation time `t` is expected to be in range `0.0..=1.0`.
    pub fn get(&self, interval: usize, t: f32) -> Option<T> {
